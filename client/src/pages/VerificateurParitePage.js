...
function VerificateurParitePage() {
    ...
    const init = async () => {
      try {
        ...
        if (deployedNetwork) {
          ...
          setError("Le contrat VerificateurParite n'est pas déployé sur ce réseau.");
        }
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    try {
      // Using send to make a real transaction
      const receipt = await contract.methods.estPair(nombre).send({ from: accounts[0] });

      // You can get the result again by call (if your contract stores state or emits events, you could read from it)
      const parityResult = await contract.methods.estPair(nombre).call();

      setResult(parityResult ? "Pair" : "Impair");
  ...
  return (
    <div style={styles.container}>
      <div style={styles.heading}>Exercice 7 : Vérificateur Parité</div>
      ...
      {transaction && (
        <div style={styles.section}>
          ...
          <p><strong>Gas utilisé:</strong> {transaction.gas}</p>
          <p><strong>Prix du Gas:</strong> {web3.utils.fromWei(transaction.gasPrice, "gwei")} Gwei</p>
        </div>
      )}
